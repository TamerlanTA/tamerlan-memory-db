...
- [x] Site-wide card polish — unified neutral `border-[#101728]/[0.06]` card borders (replacing per-category colored borders) on homepage/`/os/[slug]`/`/stacks`/pipeline cards — committed `ba2e6ac`, July 3, 2026
- [x] `CardWatermark` numbered-watermark motif — extracted to shared `src/components/CardWatermark.tsx`; live on homepage (`SafeDeploymentSection`, `DeploymentScenariosSection`, proof cards), `/stacks` bundle cards (own inline implementation, pre-existing), `/os/[slug]` Payback signal + After FlowOps panels, and `PipelineCard`/`ComingSoonPipelineCard` on `/os` — the last two were claimed done in the `ba2e6ac` session note but were not actually implemented in code (`ba2e6ac` only touched card borders + homepage watermark); gap found and fixed and committed `a4d4163`, July 3, 2026. Not yet deployed to production. See [[sessions/2026-07-03-card-watermark-rollout-fix]] and [[sessions/2026-07-03-commit-watermark-fix]].
