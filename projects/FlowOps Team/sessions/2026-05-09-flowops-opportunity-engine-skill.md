...
- 2026-05-18 update: every use of the skill must now persist newly returned leads into `[[../../../Leads/flowops-opportunity-registry]]` and dedupe future outputs against the `Leads/` memory folder so repeated calls return net-new companies by default.
...
- Use normalized company domain as the primary dedupe key before returning future lead batches.
